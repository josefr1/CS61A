...
def totals_tree(m):
    ...
    if is_planet(m):
        return tree(size(m))
    new_root = [totals_tree(end(b)) for b in [left(m), right(m)]]
    return tree(total_weight(m), new_root)
...
def replace_thor_at_leaf(t, thors_replacement):
    ...
    replacement = [replace_thor_at_leaf(a, thors_replacement) for a in branches(t)]
